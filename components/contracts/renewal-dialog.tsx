...
export function RenewalDialog({ contractId, open, onClose }: RenewalDialogProps) {
  ...
  // Update form when renewal data changes
  useEffect(() => {
    if (renewalData) {
      form.reset({
        subStatus: renewalData.subStatus,
        proposedStartDate: renewalData.proposedStartDate.split('T')[0],
        proposedEndDate: renewalData.proposedEndDate.split('T')[0],
  ...
  const onSubmit = async (data: RenewalFormData) => {
    ...
    try {
      const url = renewalData?.id 
        ? `/api/contracts/${contractId}/renewal`
        : `/api/contracts/${contractId}/renewal`;
      ...
      if (response.ok) {
        ...
        toast.error(error.message || 'Failed to save renewal');
      }
    } catch (error) {
    ...
    for (const file of Array.from(files)) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          await loadRenewalData(); // Refresh data to show new attachment
        } else {
          const error = await response.json();
